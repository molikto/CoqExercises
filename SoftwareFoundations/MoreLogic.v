...
Theorem andb_true_r: forall a b, andb a b = true -> a = true.
Proof. intros. induction a. trivial. simpl in H. inversion H. Qed.


Theorem andb_true_l: forall a b, andb a b = true -> b = true.
Proof. intros. induction b. trivial. induction a. simpl in H. inversion H. simpl in H. inversion H. Qed.

Goal forall X t l, @forallb X t l = true -> all X (fun x => t x = true) l.
Proof. intros. induction l. constructor. simpl in H. set H. apply andb_true_l in e. set H. apply andb_true_r in e0. apply IHl in e. constructor. trivial. trivial. Qed. 
...
Theorem iom_nil_r: forall T l l3, @iom T l [ ] l3 -> l = l3.
Proof. intros. dependent induction H. trivial. rewrite IHiom. trivial. Qed.  


Goal forall T test l1 l2 l3,
  all T (fun e => test e = true) l1 -> 
    all T (fun e => test e = false) l2 -> 
      iom l1 l2 l3 ->
        l1 = filter test l3.
Proof. intros. induction H1. trivial. inversion H. set (IHiom H5 H0). unfold filter. rewrite H4. rewrite e0. trivial. inversion H0. set (IHiom H H5). rewrite e0. unfold filter. rewrite H4. trivial. Qed.
...
Goal forall r l test,
  all nat (fun e => test e = true) r -> subseq r l -> length r <= length (filter test l).
Proof. intros. 
induction H0. 
simpl. 
constructor. 
...
induction b.
 ...
 rewrite Heqb.
  simpl. 
  refine (le_trans _ (length (filter test r)) _ _ _). 
  trivial. 
...
Theorem cons_app_assoc: forall T (a:T) l1 l2, (a:: l1) ++ l2 = a :: l1 ++ l2.
Proof. intros. assert ( a :: l1 ++ l2 = [a] ++ l1 ++ l2). apply cons_to_app. rewrite H. rewrite cons_to_app. rewrite <- Coq.Lists.List.app_assoc_reverse. trivial. Qed.

Lemma appears_in_app : forall (X:Type) (xs ys : list X) (x:X), 
     appears_in x (xs ++ ys) -> appears_in x xs \/ appears_in x ys.
Proof. intros. induction xs. right. simpl in H. trivial. rewrite cons_app_assoc in H. inversion H. left. constructor. apply IHxs in H1. inversion H1. left. constructor. trivial. right. trivial. Qed.

Lemma app_appears_in : forall (X:Type) (xs ys : list X) (x:X), 
     appears_in x xs \/ appears_in x ys -> appears_in x (xs ++ ys).
Proof. intros. induction xs. inversion H. inversion H0. trivial. inversion H. inversion H0. subst. rewrite cons_app_assoc. constructor. rewrite cons_app_assoc. constructor.  assert( appears_in x xs \/ appears_in x ys). left. trivial. apply IHxs in H4. trivial. rewrite cons_app_assoc. assert (appears_in x xs \/ appears_in x ys ). right. trivial. apply IHxs in H1. constructor. trivial. Qed.
...
| no_cons : forall l (e : X), ~(appears_in e l) -> no_repeats l -> no_repeats (e :: l).
...
Goal forall T l1 l2,
  @no_repeats T l1 -> 
    no_repeats l2 -> 
      disjoint T l1 l2 -> 
        no_repeats (l1 ++ l2).
Proof. intros. induction l1, l2. trivial. trivial. rewrite Lists.List.app_nil_r. trivial. admit. Qed.
